...
// Get friend code of chat partner if in chat
$chat_partner_code = '';
if ($chat_with && isset($chat_users[$chat_with])) {
    $chat_partner_code = $chat_users[$chat_with]['friend_code'];
}
?>
...
<?php else: ?>
    <p>
        ...
        <?php else: ?>
            <span style="color:red;">(Friend code not set for chat. Try logging out and back in.)</span>
        <?php endif; ?>
    </p>
 <div class="logout">
...
document.querySelectorAll('.chat-user-link').forEach(link => {
    ...
    <div class="chat-box">
        <?php if ($chat_with && !empty($chat_partner_code)): ?>
            <div style="text-align:center; margin-bottom:10px; color:#0095f6; font-weight:600;">
                Friend Code: <?= htmlspecialchars($chat_partner_code) ?>
            </div>
        <?php endif; ?>
